 ...
 */
function extractMaterialReportLink(
  html: string,
  ...
  $("a").each((index, element) => {
    ...
    if (linkText.includes("Material Procured Report")) {
      ...
      if (href) {
        ...
        if (
          href.includes("state_html/gpwrkbilldtl.aspx") &&
          !href.includes("Found material report link")
        ) {
          materialLink = href;
  ...
  if (!materialLink) {
    console.log(
      "Constructing Material Procured Report link manually due to corrupted HTML"
    );
    materialLink =
      `state_html/gpwrkbilldtl.aspx?page=P&file=dmd&lflag=eng` +
      `&fin_year=${finYear}` +
      `&state_code=15` +
      `&district_code=${panchayatData.district_code}` +
      `&block_code=${panchayatData.block_code}` +
...
function parseMaterialData(
  ...
  const allText = $.text();
  console.log(
    "First 2000 characters of HTML text:",
  ...
  if (allText.includes(workCode)) {
    ...
    console.log(
      `Checking for parts - Panchayat: ${panchayatCode}, Type: ${workType}, ID: ${workId}`
    );
  ...
  $("table tr").each((index, row) => {
    ...
    if (rowText.includes(workCode)) {
      console.log(
        `✓ Found work code in row ${index}: ${rowText.substring(0, 200)}`
      );
      workFound = true;
      currentTableRows = [];
    ...
    if (workFound) {
      ...
      const otherWorkCodePattern = new RegExp(
        `${currentWorkCodePanchayat}/[^/]+/`,
        "g"
      );
      ...
      if (matches && matches.length > 0) {
        ...
        for (const match of matches) {
          if (!workCode.includes(match.slice(0, -1))) {
  ...
  if (!workFound) {
    ...
    $("table tr, div, p, span").each((index, element) => {
      ...
      if (elementText.includes(workCode)) {
        console.log(
          `✓ Found work code in element ${element.tagName} at index ${index}: ${elementText.substring(0, 200)}`
        );
  ...
  currentTableRows.forEach((row, rowIndex) => {
    ...
    console.log(
      `Row ${rowIndex}: ${cells.length} cells - ${rowText.substring(0, 100)}`
    );
    ...
    if (
      ...
      (rowText.includes("Rupees") || rowText.includes("Quantity"))
    ) {
      isHeaderRow = true;
  ...
  console.log(
    `Final result: ${materials.length} materials, total: ${result.totalAmount}`
  );
...
materialMisRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const materialReportLink = extractMaterialReportLink(
        mainPageResponse.data,
        panchayatData,
        finYear
      );
      ...
      try {
        ...
        try {
          ...
          console.log(
            `Response preview: ${materialPageResponse.data.substring(0, 300)}`
          );
          ...
          console.log(
            `Without Digest response: ${materialPageResponse.status}`
          );
          console.log(`Response length: ${materialPageResponse.data.length}`);
          console.log(
            `Response preview: ${materialPageResponse.data.substring(0, 300)}`
        ...
        if (
          ...
          responseText.includes("Please Re-Login")
        ) {
          console.log(
            "❌ Session expired - this indicates server-side restrictions"
          );
          ...
          !responseText.includes("Bill No") &&
          !responseText.includes("Material")
        ) {
          console.log(
            "❌ Response does not contain expected material data structure"
          );
          ...
          if (
            ...
            console.log(
              "✅ Valid HTML page received, but no material data found"
            );
        ...
        console.log(
          "✅ Received valid response, checking for work code data..."
        );

        // Check if our specific work code is in the response
        if (!responseText.includes(workCode)) {
          console.log(
            `❌ Work code ${workCode} not found in the material report`
          );
      ...
      if (!materialPageResponse) {
        throw new Error(
          "Failed to fetch material report after multiple attempts"
        );
      }

      const materialData = parseMaterialData(
        materialPageResponse.data,
        workCode
      );
