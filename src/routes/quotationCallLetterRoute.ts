...
import { MaterialData, QuotationCallData, QuotationCallResponse, scrapeAdministrativeSanctionNumber, scrapeTechnicalEstimateMaterialData, validateAndCleanMaterialData } from "../services/quotationCallLetterService";

const quotationCallRouter = express.Router();
/**
 * Main API endpoint - Get quotation call data by work detail ID
 */
quotationCallRouter.get("/get-quotation-call/:id", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    if (workDocument.technicalEstimate) {
      console.log(`Fetching material data from: ${workDocument.technicalEstimate}`);
      materialData = await scrapeTechnicalEstimateMaterialData(workDocument.technicalEstimate);
    ...
    if (workDocument.administrativeSanction) {
      console.log(`Fetching administrative sanction from: ${workDocument.administrativeSanction}`);
    ...
    if (materialData.length === 0) {
      ...
      res.status(404).json({
        success: false,
        error: "No material data found in the technical estimate. Please check the URL and try again.",
 ...
 */
quotationCallRouter.get("/get-quotation-call-by-code/:workCode", async (req: Request, res: Response) => {
  try {
